<template>
  <Teleport to="body">
    <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div
        class="absolute inset-0 bg-black/85 backdrop-blur-sm t-modal-backdrop"
        :class="{ 'is-open': isOpen, 'is-closing': isClosing }"
        @click="emit('close')"
      ></div>

      <div
        class="vault-panel relative rounded-2xl p-6 md:p-8 w-full max-w-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col t-modal"
        :class="{ 'is-open': isOpen, 'is-closing': isClosing }"
      >
                ...
                <div class="border-t border-dashed border-vault-border/60 pt-3 mt-2">
                  ...
                  <div class="flex justify-between items-center">
                    <span class="font-display text-2xl lg:text-3xl gold-text font-bold">
                      {{ config.currency }}<DigitDisplay :value="totalCompensation.toLocaleString()" :pad="false" />
                    </span>
...
const socialAverage = ref('')
const yearsMode = ref('direct')
...
const totalCompensation = computed(() => {
  ...
  return Math.round((baseN * multiplier) + plusOneCompensation.value)
})
</script>
